    ...
    return (
        <div className="tasks-box">
            ...
            <form className="tasks__form" onSubmit={handleSubmit(submitForm)}>
                ...
                {errors.newTask ? (
                    <span className="err-msg">Matn kiritilishi shart</span>
                ) : null}
                <span className="current-date">
                    Bugun: {setDate().day}.{setDate().month}.{setDate().year},{" "}
                    {setDate().hour}:{setDate().minute}
                </span>
            </form>
            {newTask ? (
                <TasksList newTask={newTask} date={setDate} />
            ) : (
                <h4 className="task__empty">Hozircha bo'sh</h4>
            )}
        </div>
    );
};
